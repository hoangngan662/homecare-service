<template>
    <div class="mt-2 relative p-4 overflow-x-auto shadow-md sm:rounded-lg">
        <h3 class="text-2xl font-extrabold dark:text-white">Hồ sơ khám bệnh</h3>
        <br />
        <div v-if="isLoading" class="text-center">Loading...</div>
        <div v-else>
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <section v-for="record in patientRecords" :key="record.serviceType"
                        ...
                        <div class="flow-root mx-auto">
                            <div v-for="(appointment, index) in record.appointments" :key="appointment.id"
                                ...
                                <div class="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-6">
                                    <div class="w-32 text-lg font-normal text-gray-500 sm:text-right dark:text-gray-400 shrink-0">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-gray-700 dark:text-gray-400">
                                            Ngày hẹn đề xuất: {{ formatDate(appointment.appointmentDetailDto.nextAppointmentDate) }}
                                        </p>
                                    </div>
                                </div>
                            </div>
...
export default {
    ...
    props: {
        patientId: {
            type: String,
            required: true // Đảm bảo rằng patientId là bắt buộc
        }
    },
    data() {
        return {
            ...
            isLoading: true // Biến để theo dõi trạng thái tải
        };
    },
    methods: {
        async fetchPatientRecords() {
            try {
                const response = await fetch(`https://localhost:7072/api/appointment/record?patientId=${this.patientId}`);
                ...
                this.isLoading = false; // Đặt isLoading thành false khi đã hoàn tất
            }
        },
