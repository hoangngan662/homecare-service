...
<template>
    <div class="relative p-4 overflow-x-auto shadow-md sm:rounded-lg">
        <h3 class="text-2xl font-extrabold dark:text-white">Danh sách nhân viên</h3>
        <p class="my-4 text-sm text-gray-500">Trang danh sách nhân viên hiển thị thông tin tổng quan và cung cấp các
            chức năng quản lý, giúp dễ dàng xem chi tiết và chỉnh sửa thông tin của từng nhân viên.</p>
        ...
        <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
            ...
            <tbody>
                <tr v-for="(staff, index) in staffs" :key="staff.id"
                    ...
                    <td class="px-6 py-4">
                        <span v-if="staff.roles[0] === 'Staff'">Nhân viên</span>
                        <span v-else-if="staff.roles[0] === 'Admin'">Quản trị viên</span>
                        <span v-else>{{ staff.roles[0] }}</span>
                    </td>
                    <td class="px-6 py-4">
