...
<template>
    <div class="relative p-4 overflow-x-auto shadow-md sm:rounded-lg">
        <h3 class="text-2xl font-extrabold dark:text-white">Danh sách loại dịch vụ</h3>
        <p class="my-4 text-sm text-gray-500">Trang danh sách loại dịch vụ hiển thị các loại dịch vụ hiện có và cung cấp
            các chức năng quản lý, cho phép người dùng xem chi tiết thêm mới, và chỉnh sửa thông tin từng
            loại dịch vụ.</p>
        <router-link to="/service-type/add"
        ...
        <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
            ...
            <tbody>
                <tr v-for="(type, index) in serviceTypes" :key="type.id"
                    ...
                    <td class="px-6 py-4 truncate-2-lines">
                        {{ truncateText(type.description) }}
                    </td>
                    <td class="px-6 py-4">
                        <a :href="`/service-type/${type.id}`"
