...
export default {
    ...
    methods: {
        async fetchServicePackages() {
            try {
                const response = await axios.get('https://localhost:7072/api/service/service-packages', {
                    params: {
                ...
                this.servicePackages = response.data.items;
...
<template>
    <div class="relative p-4 overflow-x-auto shadow-md sm:rounded-lg">
        <h3 class="text-2xl font-extrabold dark:text-white">Danh sách gói dịch vụ</h3>
        <p class="my-4 text-sm text-gray-500">Trang danh sách gói dịch.</p>
        ...
        <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
            ...
            <tbody>
                <tr v-for="(pack, index) in servicePackages" :key="pack.id"
                    ...
                    <th class="px-6 py-4">
                        <span>{{ pack.serviceTypeName }}</span>
                        <br/>
                        <span class="py-4 font-medium text-gray-900 dark:text-white">{{ pack.name }}</span>
                    </th>
                    <td class="px-6 py-4">
                        {{ pack.numberOfSessions }} buổi / {{ pack.durationInMonths }} tháng
                    </td>
                    <td class="px-6 py-4">
                        {{ pack.subscriptionCount }}
                    </td>
                    <td class="px-6 py-4">
