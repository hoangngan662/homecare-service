    ...
    methods: {
        async fetchData() {
            try {
                ...
                const response = await axios.get('https://localhost:7072/api/subscription/patient-paid', {
                    params: {
                        patientId: patientId,
...
<template>
    ...
    <div class="relative m-4 p-4 overflow-x-auto shadow-md sm:rounded-lg">
        <h3 class="text-2xl font-extrabold dark:text-white">Thanh toán dịch vụ</h3>
        <br />
        <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
            ...
            <tbody>
                <tr v-for="(sub, index) in subscriptions" :key="sub.id"
                    ...
                    <td class="px-6 py-4">
                        {{ new Date(sub.paymentDate).toLocaleDateString('en-GB') }}
                        <br />
                        {{ new Date(sub.paymentDate).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })
                        }}
                    </td>
                    <td class="px-6 py-4">
                        {{ formatPrice(sub.total) }} VNĐ
                    </td>
                </tr>
            </tbody>
        </table>
